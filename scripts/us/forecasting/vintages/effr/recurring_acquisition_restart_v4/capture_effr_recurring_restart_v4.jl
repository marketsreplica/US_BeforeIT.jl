...
function usage(io::IO = stdout)
    return println(
        ...
        """
        Usage:
          capture_effr_recurring_restart_v4.jl \\
        ...
        Dates, effective dates, windows, transaction IDs, queries, the exact
        data/us/raw/forecasting/effr/prospective/2026q3_restart_v2 output root,
        paths, and predecessors are derived from the frozen restart schedule and
...
function print_plan(plan)
    println("EFFR restart-bound recurring acquisition plan")
    ...
    return println(
        "  Frozen restart-schedule network gate: ",
        plan.operator_authorization[
...
function main(args = ARGS)
    ...
    try
        result = acquire_restart_recurring(
            parsed.publication_date,
            parsed.phase;
            execute_live = parsed.execute_live,
        )
        if parsed.execute_live
            println("EFFR restart-bound recurring acquisition completed")
