...
struct ProductivityShock <: AbstractShock
...
function (s::ProductivityShock)(model::Bit.AbstractModel)
    return if model.agg.t == 1
        model.firms.alpha_bar_i .=
            model.firms.alpha_bar_i .* s.productivity_multiplier
    end
end
...
function (s::ConsumptionShock)(model::Bit.AbstractModel)
    return if model.agg.t == 1
        model.prop.psi = model.prop.psi * s.consumption_multiplier
    elseif model.agg.t == s.final_time + 1
        model.prop.psi = model.prop.psi / s.consumption_multiplier
    end
end
